    ...
    return <nav className="bg-white shadow-md py-4 border-b border-gray-200">
        ...
        <div className="container mx-auto flex justify-between items-center px-6 lg:px-8">
            <Link href={"/"} className="flex items-center">
                ...
                {" "}
                <span className="font-bold text-2xl text-gray-800">Travel Buddy</span>
            </Link>

            <div className="flex items-center space-x-4">
                {session? (
                    <>
                <Link href={"/trips"} className=" text-slate-900 hover:text-sky-500">Trips</Link>
                <Link href={"/globe"} className=" text-slate-900 hover:text-sky-500">Globe</Link>

                <button className="flex justify-center items-center bg-gray-800 hover:bg-gray-900 text-white p-2 rounded-sm cursor-pointer" onClick={logout}>Sign Out</button>
                </>
                ):(
                <button className="flex justify-center items-center bg-gray-800 hover:bg-gray-900 text-white p-2 rounded-sm cursor-pointer" onClick={login}>
                    Sign In
                    <svg
